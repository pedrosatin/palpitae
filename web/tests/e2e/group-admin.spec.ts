  ...
  test('Deve visualizar abas e editar grupo como admin', async ({ page }) => {
    ...
    await page.route('**/api/groups**', async route => {
      ...
      if (method === 'GET' && url.endsWith('/members')) {
        ...
        await route.fulfill({
          headers: corsHeaders,
          json: { group: { id: 'group-1', name: 'Grupo Original', is_admin: true, competition_name: 'Brasileirão' } }
        });
      } else if (method === 'PATCH' && url.includes('/group-1')) {
        patchCalled = true;
        await route.fulfill({
          headers: corsHeaders,
          json: { group: { id: 'group-1', name: 'Grupo Editado', is_admin: true } }
        });
      } else {
  ...
  test('Deve ver apenas a opção de sair do grupo como membro comum', async ({ page }) => {
    ...
    await page.route('**/api/groups**', async route => {
      ...
      if (method === 'DELETE' && url.includes('/members/user-1')) {
        leaveCalled = true;
        await route.fulfill({ status: 200, headers: corsHeaders, json: { success: true } });
      } else if (method === 'GET' && url.endsWith('/group-1')) {
        await route.fulfill({
          headers: corsHeaders,
          json: { group: { id: 'group-1', name: 'Grupo Original', is_admin: false, competition_name: 'Brasileirão' } }
        });
      } else {
        await route.continue();
      }
    ...
    await page.getByRole('dialog').getByRole('button', { name: /Sair|Confirmar/i }).click();
    ...
    await page.waitForResponse(response => response.url().includes('/members/user-1') && response.status() === 200);
