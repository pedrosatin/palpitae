...
export function groupedByGroupName(
  ms: UserPrediction[],
): [string | null, UserPrediction[]][] {
...
export default function PredictionsModal({
  ...
  const safeModalIndex = Math.min(
    modalRoundIndex,
    Math.max(0, modalRoundKeys.length - 1),
  );
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={`Palpites de ${member.display_name}`}
    >
      {modalLoading && (
        ...
        <>
          ...
          <div className={styles.predGroups}>
            {groupedByGroupName(modalByRound.get(selectedModalRound) ?? []).map(
              ([groupName, matches]) => (
                <div
                  key={groupName ?? "__no_group"}
                  className={styles.predGroup}
                >
                  {groupName && (
                    <h4 className={styles.predGroupHeader}>
                      Grupo {groupName}
                    </h4>
                  )}
                      ...
                      return (
                        <li key={p.match_id} className={styles.predItem}>
                          <div className={styles.predMatch}>
                            ...
                            <span
                              className={`${styles.predTeam} ${styles.predTeamAway}`}
                            >
