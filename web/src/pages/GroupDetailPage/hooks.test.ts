...
import { useGroupActions, useGroupTabs } from "./hooks";
...
describe("useGroupActions", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(useConfirm).mockReturnValue({
      confirm: mockConfirm as unknown as ReturnType<
        typeof useConfirm
      >["confirm"],
  ...
  describe("leaveGroup", () => {
    ...
    it("successfully leaves group and navigates away", async () => {
      ...
      expect(apiFetch).toHaveBeenCalledWith(
        expect.stringContaining("/groups/g1/members/u1"),
        {
          method: "DELETE",
        },
      );
  ...
  describe("deleteGroup", () => {
    ...
    it("returns early if user cancels confirmation", async () => {
      ...
      expect(trackEvent).toHaveBeenCalledWith(
        "click_group_detail_menu_excluir",
      );
      expect(mockConfirm).toHaveBeenCalled();
      expect(apiFetch).not.toHaveBeenCalled();
    });
...
describe("useGroupTabs", () => {
  ...
  it("initializes with tab from URL", () => {
    const { result } = renderHook(() => useGroupTabs(mockGroup), {
      wrapper: ({ children }) =>
        React.createElement(
          MemoryRouter,
          { initialEntries: ["/?tab=leaderboard"] },
          children,
        ),
    });
  ...
  it("does not prevent default or call setActiveTab if modifier key is pressed", () => {
    ...
    act(() => {
      result.current.handleTabClick(
        { preventDefault, ctrlKey: true } as any,
        "leaderboard",
      );
    });
