...
function buildApiCallsSeries(
  ...
  const byDay = new Map(
    apiCallsDaily.map((r) => [r.day.slice(0, 10), Number(r.api_calls)]),
  );
...
function useMetricsData(
  ...
  const apiCallsSeries = useMemo(
    () =>
      overview
        ? buildApiCallsSeries(overview.apiCallsDaily, overview.days)
        : [],
    [overview],
  );
  ...
  const kpis = useMemo(() => {
    ...
    const hits = Number(
      overview.cache.find((c) => c.result === "hit")?.count ?? 0,
    );
    const misses = Number(
      overview.cache.find((c) => c.result === "miss")?.count ?? 0,
    );
    ...
    return {
      ...
      joinRate:
        groupsCreated > 0 ? (groupsJoined / groupsCreated).toFixed(1) : "—",
      logins: totalOf("login_success"),
      cacheHitRate:
        cacheTotal > 0 ? `${Math.round((hits / cacheTotal) * 100)}%` : "—",
  ...
  const avgSample = overview
    ? Number(overview.sampling.avg_sample_interval ?? 1)
    : 1;
    ...
    const shareOf = (n: number) =>
      Math.min(
        ...
        Math.round(
          (overview.whales
            .slice(0, n)
            .reduce((sum, w) => sum + Number(w.predictions), 0) /
            kpis.predictions) *
            100,
        ),
      );
  ...
  const archiveDays = useMemo(() => {
    ...
    const byDay = new Map(
      archive.files.map((f) => [
        f.key.replace("events/", "").replace(".ndjson", ""),
        f,
      ]),
    );
  ...
  const archiveStats = useMemo(() => {
    ...
    const totalEvents = archive.files.reduce(
      (sum, f) => sum + (f.events ?? 0),
      0,
    );
...
function OverviewKpisRow({
  ...
  return (
    <div className={styles.kpiRow}>
      ...
      <KpiCard
        label="entradas por grupo"
        value={kpis.joinRate}
        hint="conversão de convite"
      />
  ...
  return (
    <>
      <section className={styles.section}>
        ...
        <StackedBarChart
          days={stacked.series}
          ariaLabel="Eventos por dia, por tipo"
        />
        ...
        <table className={styles.table}>
          <tbody>
            {overview.lastRuns.map((r) => {
              ...
              const stale = info
                ? isStale(r.last_run, info.staleAfterMin)
                : false;
              return (
                <tr key={r.event_type}>
                  <td>{info?.label ?? r.event_type}</td>
                  <td
                    className={`${styles.num} ${stale ? styles.statusError : ""}`}
                  >
          ...
          <tbody>
            {overview.poller.map((p) => (
              <tr key={p.status}>
                <td
                  className={
                    p.status === "error" ? styles.statusError : styles.statusOk
                  }
                >
                  {p.status}
                </td>
                ...
                <td className={styles.num}>
                  {formatDuration(Number(p.avg_duration_ms))}
                </td>
                <td className={styles.num}>
                  {formatDuration(Number(p.max_duration_ms))}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <table className={styles.table}>
        <tbody>
          <tr>
            ...
            <td className={styles.num}>
              {Number(overview.emailHealth.rounds)}
            </td>
          </tr>
          <tr>
  ...
  return (
    <section className={styles.section}>
      ...
      <p className={styles.hint}>
        Maior palpiteiro: <strong>{whaleShare.top1}%</strong> dos palpites · top
        5: <strong>{whaleShare.top5}%</strong> — quanto mais alto, mais a
        retenção depende de poucos usuários hardcore.
      </p>
    </section>
  );
}
...
function ArchiveSection({
  ...
  return (
    <section className={styles.section}>
      ...
      {!archiveStats ? (
        <p className={styles.hint}>
          Nenhum arquivo no bucket — em dev local o R2 é simulado e começa
          vazio; em produção, ver se o cron de export (00:05 UTC) está rodando.
        </p>
      ) : (
        <>
          <div className={styles.kpiRow}>
            <KpiCard
              label="dias arquivados"
              value={String(archiveStats.totalDays)}
            />
            <KpiCard
            ...
            <KpiCard
              label="tamanho total"
              value={formatBytes(archiveStats.totalBytes)}
            />
            <KpiCard label="desde" value={archiveStats.oldestDay} />
          </div>

          <h3 className={styles.subTitle}>
            Eventos por mês · histórico completo
          </h3>
          <BarChart
          ...
          {archiveStats.missingMeta > 0 && (
            <p className={styles.hint}>
              ...
              {archiveStats.missingMeta === 1
                ? "arquivo ainda sem"
                : "arquivos ainda sem"}{" "}
              contagem de eventos (export antigo) — o cron re-grava a metadata
              aos poucos; até lá os totais acima são piso ("≥").
            </p>
          )}
              ...
              {archiveDays.map(({ day, file, beforeFirstExport }) => (
                <tr key={day}>
                  ...
                  {file ? (
                    ...
                    <td
                      className={`${styles.num} ${styles.statusError}`}
                      colSpan={2}
                    >
                      faltando
                    </td>
          ...
          <p className={styles.hint}>
            <strong>faltando</strong> = o export diário não gravou o NDJSON
            desse dia (cron falhou; o backfill re-tenta sozinho enquanto o dia
            estiver na janela de ~3 meses do Analytics Engine). "—" = dia
            anterior ao primeiro export.
          </p>
        </>
      )}
    </section>
          ...
          <>
            ...
            {isSampling && (
              <p className={styles.samplingWarning}>
                ...
                {avgSample.toFixed(2)}) — os números do período são estimativa e
                o volume de eventos merece atenção.
              </p>
            )}
