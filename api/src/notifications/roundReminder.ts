...
async function sendWithRetry(apiKey: string, msg: EmailMessage): Promise<void> {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      const retryAfterMs =
        err instanceof EmailError ? err.retryAfterMs : undefined;
...
export async function sendRoundReminders(
  ...
  if (!resendApiKey.trim()) {
    console.error(
      "[roundReminder] RESEND_API_KEY ausente — nenhum lembrete enviado.",
    );
  ...
  const apiBaseUrl = unsub?.apiBaseUrl
    ? unsub.apiBaseUrl.replace(/\/+$/, "")
    : undefined;
    ...
    for (const {
      ...
      const p = (async () => {
        ...
        try {
          ...
          failed++;
          console.error(
            `[roundReminder] Falha ao enviar para ${hashedId}:`,
            err,
          );
        }
      })();
  ...
  logEvent(ae, "cron_round_reminder", {
    doubles: [grouped.size, sent, failed],
  });
