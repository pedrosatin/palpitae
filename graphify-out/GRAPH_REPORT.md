# Graph Report - palpitae  (2026-06-29)

## Corpus Check
- 154 files · ~87,315 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 868 nodes · 1177 edges · 71 communities (66 shown, 5 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `da16bb3f`
...
Cohesion: 0.05
Nodes (58): fakeEnv(), requestWithCookie(), competitions, result, router, ActiveCompetition, discoverFixtures(), ActiveRound (+50 more)
...
Cohesion: 0.18
Nodes (9): HMAC_SHA256, importHmacKey(), JwtPayload, authHeaders(), Captured, signUnsubToken(), forgedId, [, sig] (+1 more)
...
Cohesion: 0.22
Nodes (6): useConfirm(), Member, MembersTab(), MembersTabProps, members, mockTrackEvent
...
## Knowledge Gaps
- **485 isolated node(s):** `PackageJson`, `Variables`, `env`, `ctx`, `app` (+480 more)
...
- **Why does `AppContext` connect `Bracket UI Components` to `Create Group Modal (legacy)`, `Dashboard Page Barrel`, `Predictions Tab (legacy)`, `Bracket API Router`, `App Entry & Login`, `Fixture Sync (API Football)`, `Create Group Modal`, `Header Tests (legacy)`, `Card (legacy)`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `signJwt()` connect `App Entry & Login` to `Bracket UI Components`, `Fixture Sync (API Football)`, `Header Tests (legacy)`, `Group Detail Page Tests (legacy)`, `Card (legacy)`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `sendRoundReminders()` connect `Bracket UI Components` to `Architecture Decision Records`, `Fixture Sync (API Football)`?**
