...
Cohesion: 0.11
Nodes (11): ConfirmModalProps, DashboardPage(), DashboardPageProps, User, useDocumentTitle(), LoginPage(), mockTrackEvent, SettingsPage() (+3 more)
...
Cohesion: 0.15
Nodes (15): 1. Configurar domínio de envio no Resend, 2. Adicionar `RESEND_API_KEY` ao projeto, 2. `api/src/matches/poller.ts` (implementado), 2. Novo arquivo: `api/src/matches/poller.ts`, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM), 3. `api/src/index.ts` — exportar handler `scheduled`, 3. `api/src/index.ts` — handler `scheduled` (implementado), 3. Duas queries D1 sequenciais por cache miss — usar `db.batch()` (+7 more)
...
Cohesion: 0.18
Nodes (14): 1. `GroupDetailPage.tsx` — adicionar a aba, 2. Segurança — XSS no template HTML (BUG), 3. Criar módulo de notificação, 3. Observabilidade — nenhum evento de negócio registrado (GAP), 3. Recipient dedup uses email instead of user_id — wrong unsubscribe token (MEDIUM), 3. `StandingsTab.test.tsx` — testes, 4. `index.ts` — barrel export, 4. Registrar cron diário no `index.ts` (+6 more)

### Community 31 - "Icon SVG Assets"
Cohesion: 0.14
Nodes (14): API externa, Arquivos novos a criar, Arquivos tocados, context.md — Tarefa atual: Result Sync & Scoring, Cooldown / proteção de rate limit, Decisões, Fluxo acordado, Janela de tempo (+6 more)
...
Cohesion: 0.15
Nodes (13): 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, Arquivos novos/modificados nessa branch, Arquivos relevantes, Checklist de merge, Contexto: Notificação de Nova Rodada por E-mail, Idempotência (sem tabela de controle) (+5 more)
...
Cohesion: 0.18
Nodes (12): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. `await hashUserId` inside `catch` aborts batch (HIGH), 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 2. Query de `default_round` ignora filtros `?round=X` / `?status=X`, 5. `makeMatch` e `mockFetch` desatualizados em `GroupPicksTab.test.tsx`, Arquivos a criar, Arquivos a modificar (+4 more)
...
Cohesion: 0.2
Nodes (11): 7. `.dev.vars` — não está no repositório, Arquivos a criar/modificar, code:ts ((env.RESEND_API_KEY ?? '').trim(),), code:ts (if (!resendApiKey.trim()) { ... }), Como testar localmente, Decisões em aberto, Decisões resolvidas na implementação, Pendências externas (não-código) (+3 more)
...
- **Why does `config` connect `Button & Header (legacy)` to `Bracket UI Components`, `Web Vite Config`, `Vite Env Types`, `Create Group Modal (legacy)`, `Group Detail Page Barrel`, `App Entry & Login`, `Community 41`, `Community 42`, `Group Card (legacy)`, `Community 44`, `Fixture Sync (API Football)`, `Create Group Modal`, `Header Tests (legacy)`, `Community 51`, `Dashboard Page Tests (legacy)`, `Community 59`, `Brand SVG Assets`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `AppContext` connect `API Routers & Auth Middleware` to `Auth Encoding & Google OAuth`, `Dashboard & Group Detail Pages`, `Bracket API Router`, `Match Card Component`, `Predictions Tab Component`, `Join Group Modal`, `Modal Component`?**
