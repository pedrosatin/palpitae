...
## Corpus Check
- 125 files · ~68,012 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1294 nodes · 2231 edges · 84 communities (78 shown, 6 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 12 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1bc4414b`
...
Cohesion: 0.05
Nodes (71): signJwt(), requireAuth, app, buildApp(), fakeEnv(), requestWithCookie(), fakeEnv(), requestWithCookie() (+63 more)

### Community 2 - "Auth Encoding & Google OAuth"
Cohesion: 0.08
Nodes (49): base64UrlDecode(), base64UrlEncode(), buf, decoded, encoded, original, result, buildAuthUrl() (+41 more)
...
Cohesion: 0.06
Nodes (53): API Football (External Provider), Cloudflare D1 (SQLite), Cloudflare Pages, Cloudflare Workers, Architecture Decision Records, Auth Domain Module, Competitions Domain Module, Groups Domain Module (+45 more)

### Community 5 - "Dashboard & Group Detail Pages"
Cohesion: 0.06
Nodes (21): ButtonProps, onClick, Competition, CreatedGroup, CreateGroupModalProps, HeaderProps, User, JoinedGroup (+13 more)
...
Cohesion: 0.08
Nodes (14): DashboardPage(), DashboardPageProps, User, GroupDetail, GroupDetailPage(), GroupDetailPageProps, parseTab(), Tab (+6 more)
...
Cohesion: 0.12
Nodes (13): trackEvent(), ConfirmModalProps, Header(), HeaderProps, User, JoinedGroup, JoinGroupModal(), JoinGroupModalProps (+5 more)
...
Cohesion: 0.12
Nodes (16): code:ts (import { hashUserId, logEvent } from '../observability'), code:ts (logEvent(c.env.AE, 'group_deleted', { blobs: [groupId, await), Como executar (paralelismo + contexto limitado), Contexto compartilhado (todas as tarefas), improvements.md — correções da observability (Fases 2–4), Itens conscientemente fora de escopo (não fazer), Onda 1 — Fundação (1 agente, roda primeiro), Onda 2 — Correções e testes (N agentes em paralelo) (+8 more)
...
Cohesion: 0.21
Nodes (9): ActiveRound, pollActiveMatches(), EventDims, EventType, hashUserId(), logEvent(), { ae, calls }, long (+1 more)
...
Cohesion: 0.17
Nodes (11): graphify-out/GRAPH_REPORT.md, Analytics — eventos de clique (obrigatório), Analytics — regra obrigatória para eventos de clique, code:ts (import { trackEvent } from '../../analytics/ga'), code:ts (import { hashUserId, logEvent } from '../observability'), graphify, Observabilidade — eventos server-side (obrigatório), Observabilidade (logs, métricas, eventos) (+3 more)
...
Cohesion: 0.22
Nodes (10): 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 2. Novo arquivo: `api/src/matches/poller.ts`, 3. Duas queries D1 sequenciais por cache miss — usar `db.batch()`, 5. Atualizar `web/index.html`, 5. `makeMatch` e `mockFetch` desatualizados em `GroupPicksTab.test.tsx`, 7. Criar `web/public/robots.txt`, code:html (<!doctype html>), code:block5 (User-agent: *) (+2 more)
...
Cohesion: 0.2
Nodes (10): 1. `api/wrangler.toml` — adicionar Cron Trigger, 2. `api/src/matches/poller.ts` (implementado), 2. Query de `default_round` ignora filtros `?round=X` / `?status=X`, 3. `api/src/index.ts` — handler `scheduled` (implementado), 4. `api/src/matches/router.ts` — sem mudança, 4. `api/src/matches/router.ts` — sem mudança agora, 4. Criar `web/public/og-image.png`, Arquivos a modificar (+2 more)
...
## Knowledge Gaps
- **397 isolated node(s):** `app`, `KNOWN_OAUTH_ERRORS`, `{ db, updateRun }`, `TEAM_TRANSLATIONS`, `COMP_TRANSLATIONS` (+392 more)
...
- **Why does `config` connect `Team Badge Component` to `API Vitest Config`, `Bracket UI Components`, `Architecture Decision Records`, `Dashboard Page Barrel`, `Dashboard & Group Detail Pages`, `Login Page Barrel`, `Bracket API Router`, `Group Card (legacy)`, `Fixture Sync (API Football)`, `Join Group Modal`, `Modal (legacy)`, `Card (legacy)`, `Google Login Button`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `AppContext` connect `API Routers & Auth Middleware` to `Auth Encoding & Google OAuth`, `App Entry & Login`, `Match Card Component`, `Predictions Tab Component`, `Card Component`?**
