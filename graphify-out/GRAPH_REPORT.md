...
## Corpus Check
- 142 files · ~79,378 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1392 nodes · 2457 edges · 95 communities (89 shown, 6 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2010aa5d`
...
Cohesion: 0.05
Nodes (22): ButtonProps, onClick, CardProps, { container }, div, GroupCardProps, GroupWithStats, baseGroup (+14 more)

### Community 3 - "Architecture Decision Records"
Cohesion: 0.05
Nodes (22): Competition, CreatedGroup, CreateGroupModalProps, competitions, createdGroup, defaultProps, JoinedGroup, JoinGroupModalProps (+14 more)
...
Cohesion: 0.11
Nodes (20): makeMatch(), defaultRoundIndex(), GroupMember, GroupPicksResponse, GroupPicksTab(), GroupPicksTabProps, MemberPrediction, GroupPicksResponse (+12 more)

### Community 7 - "Bracket API Router"
Cohesion: 0.1
Nodes (21): body, competitions, createdGroup, defaultProps, exact, fetchSpy, infoBtn, mockFetchCompetitions() (+13 more)
...
Cohesion: 0.13
Nodes (17): router, dayBounds(), exportEventsToR2(), exportRecentDays(), dd, env, events, FetchFake (+9 more)
...
Cohesion: 0.12
Nodes (23): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. Configurar domínio de envio no Resend, 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 2. Adicionar `RESEND_API_KEY` ao projeto, 2. `api/src/matches/poller.ts` (implementado), 2. `api/src/matches/scoring.ts`, 2. Novo arquivo: `api/src/matches/poller.ts` (+15 more)
...
Cohesion: 0.15
Nodes (19): 10. O(n²) `Array.find()` scan in recipient dedup (LOW — efficiency), 1. `GroupDetailPage.tsx` — adicionar a aba, 2. Segurança — XSS no template HTML (BUG), 3. `api/src/groups/router.ts`, 3. Observabilidade — nenhum evento de negócio registrado (GAP), 3. Recipient dedup uses email instead of user_id — wrong unsubscribe token (MEDIUM), 3. `StandingsTab.test.tsx` — testes, 4. Duplicate `default_round` correlated subquery in Q1 and Q2 (MEDIUM — maintenance/correctness) (+11 more)
...
Cohesion: 0.24
Nodes (9): CacheEntry, fetchCachedJson(), invalidateApiCache(), responseCache, loader, defaultRoundIndex(), PredictionMap, PredictionsTab() (+1 more)
...
Cohesion: 0.2
Nodes (10): ActiveRound, pollActiveMatches(), unsubscribeByToken(), EventDims, EventType, hashUserId(), logEvent(), { ae, calls } (+2 more)
...
Cohesion: 0.22
Nodes (8): formatDate(), LeaderboardTab(), LeaderboardTabProps, Member, members, mockTrackEvent, UserPrediction, applyDefaultRound()
...
Cohesion: 0.2
Nodes (10): Arquivos novos/modificados nessa branch, Arquivos relevantes, Checklist de merge, Contexto: Notificação de Nova Rodada por E-mail, Idempotência (sem tabela de controle), Lógica central, O que já está feito, Pendências externas (processos manuais, sem código) (+2 more)
...
Cohesion: 0.38
Nodes (7): 2. `StandingsTab.tsx` — componente principal, 4. `api/src/predictions/router.ts` — `GET /predictions/group`, 7. Misconfig and "no rounds today" produce identical metrics (LOW), 9. `hashUserId(id)` called twice per failed send (LOW — efficiency), code:ts (const groupConfig = await db), code:sql (SELECT), code:ts (type ScoringPreset = 'classic' | 'exact_only' | 'winner_only)

### Community 69 - "Community 69"
Cohesion: 0.29
Nodes (7): 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, 7. `.dev.vars` — não está no repositório, code:ts (logEvent(c.env.AE, 'group_created', {), O que falta implementar, Pendências externas (não-código)

### Community 70 - "Community 70"
Cohesion: 0.29
Nodes (7): 4. `index.ts` — barrel export, 5. `web/src/components/CreateGroupModal/CreateGroupModal.tsx`, code:ts (setScoringPreset('classic')), code:ts (function handlePreset(preset: ScoringPreset) {), code:ts (if (pointsExact < pointsWinner) {), code:ts (body: JSON.stringify({), Tipos relevantes já existentes

### Community 71 - "Community 71"
Cohesion: 0.29
Nodes (7): Arquivos tocados, code:sql (g.points_exact,), Como testar localmente, Decisões, Janela de tempo, Próximos passos (após estabilizar o Cron), Tarefa atual: Cron Poller de Resultados
...
- **Why does `config` connect `Match Card Component` to `Bracket UI Components`, `Community 66`, `Web Test Setup`, `Auth Encoding & Google OAuth`, `Architecture Decision Records`, `Predictions Tab (legacy)`, `Community 41`, `Community 43`, `Join Group Modal`, `Community 49`, `Group Detail Page Tests (legacy)`, `Group Detail Page (legacy)`, `Card (legacy)`, `Card Component`, `Pages Function Proxy`, `Web App Entry`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Community 44` to `Web Vite Config`, `Vite Env Types`, `Dashboard & Group Detail Pages`, `Login Page Barrel`, `App Entry & Login`, `Predictions Tab Component`, `Group Card (legacy)`, `Community 45`, `Create Group Modal`, `Modal Component`, `Google Login Button`?**
