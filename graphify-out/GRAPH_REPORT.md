...
## Corpus Check
- 144 files · ~82,769 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1390 nodes · 2452 edges · 94 communities (87 shown, 7 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `17842996`
...
Cohesion: 0.06
Nodes (63): base64UrlDecode(), base64UrlEncode(), buf, decoded, encoded, original, result, buildAuthUrl() (+55 more)
...
Cohesion: 0.16
Nodes (11): dialog, fetchSpy, mockResponse(), mockTrackEvent, user, Member, MembersTab(), MembersTabProps (+3 more)
...
Cohesion: 0.15
Nodes (9): trackEvent(), DashboardPage(), DashboardPageProps, User, useDocumentTitle(), SettingsPage(), SettingsPageProps, User (+1 more)

### Community 16 - "Join Group Modal"
Cohesion: 0.14
Nodes (20): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. Configurar domínio de envio no Resend, 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 2. Adicionar `RESEND_API_KEY` ao projeto, 2. `api/src/matches/scoring.ts`, 2. Novo arquivo: `api/src/matches/poller.ts`, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM), 2. Query de `default_round` ignora filtros `?round=X` / `?status=X` (+12 more)
...
Cohesion: 0.15
Nodes (11): Competition, CreatedGroup, CreateGroupModal(), CreateGroupModalProps, PRESET_LABELS, PRESET_VALUES, SCORING_HELP_TEXT, ScoringPreset (+3 more)
...
Cohesion: 0.16
Nodes (15): 1. `api/migrations/0008_group_scoring_config.sql` — NOVO, 2. `StandingsTab.tsx` — componente principal, 4. `api/src/predictions/router.ts` — `GET /predictions/group`, 5. `web/src/components/CreateGroupModal/CreateGroupModal.tsx`, 6. UI simplificada de palpite (quando `points_exact = 0`), 7. Misconfig and "no rounds today" produce identical metrics (LOW), 9. `hashUserId(id)` called twice per failed send (LOW — efficiency), Arquivos a criar/modificar (+7 more)
...
Cohesion: 0.2
Nodes (10): 2. `api/src/matches/poller.ts` (implementado), 3. `api/src/index.ts` — handler `scheduled` (implementado), 4. `api/src/matches/router.ts` — sem mudança, 4. `api/src/matches/router.ts` — sem mudança agora, Arquivos tocados, Decisões, Janela de tempo, O que muda (+2 more)
...
Cohesion: 0.33
Nodes (6): 1. `await hashUserId` inside `catch` aborts batch (HIGH), Arquivos a criar, code:sql (ALTER TABLE groups ADD COLUMN points_exact INTEGER NOT NULL ), Pendências externas (processos manuais, sem código), Schema relevante, Testar localmente após setup
...
- **Why does `config` connect `Fixture Sync (API Football)` to `Community 64`, `Bracket UI Components`, `Web Vite Config`, `Create Group Modal (legacy)`, `Dashboard & Group Detail Pages`, `Community 42`, `Group Card (legacy)`, `Group Card Component`, `Community 44`, `Predictions Tab Component`, `Button & Header (legacy)`, `Modal (legacy)`, `Card (legacy)`, `Dashboard Page Tests (legacy)`, `Pages Function Proxy`, `Community 63`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `AppContext` connect `API Routers & Auth Middleware` to `Auth Encoding & Google OAuth`, `Architecture Decision Records`, `Bracket API Router`, `App Entry & Login`, `Create Group Modal`, `Team Badge Component`?**
