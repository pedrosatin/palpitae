...
## Corpus Check
- 139 files · ~71,319 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1211 nodes · 2125 edges · 84 communities (78 shown, 6 thin omitted)
...
Cohesion: 0.05
Nodes (81): base64UrlDecode(), base64UrlEncode(), buf, decoded, encoded, original, result, buildAuthUrl() (+73 more)
...
Cohesion: 0.1
Nodes (11): Competition, CreatedGroup, CreateGroupModalProps, JoinedGroup, JoinGroupModalProps, ModalProps, onClose, GroupDetail (+3 more)

### Community 13 - "Fixture Sync (API Football)"
Cohesion: 0.1
Nodes (19): A. Ativar e verificar o GA4 em produção *(ação do dono — sem código)*, ⏸️ Adiado por design — só fazer quando o gatilho existir, Analytics — GA4 com Consent Mode v2 + banner LGPD, Auditoria Lighthouse (mobile, build de prod) — 2026-06-19, B. Keyword research *(depende de tráfego acumular no Search Console)*, C. Bing Webmaster Tools *(ação do dono — sem código)*, Conteúdo / SEO de conteúdo — páginas estáticas (2026-06-20), context-seo.md — SEO, Performance & Analytics (+11 more)
...
Cohesion: 0.16
Nodes (9): ConfirmModalProps, JoinedGroup, JoinGroupModal(), JoinGroupModalProps, body, defaultProps, fetchSpy, joinedGroup (+1 more)
...
## Knowledge Gaps
- **349 isolated node(s):** `app`, `{ db, updateRun }`, `syncFixturesMock`, `scoreMock`, `ActiveRow` (+344 more)
...
- **Why does `config` connect `Card (legacy)` to `API Vitest Config`, `API Routers & Auth Middleware`, `Vite Env Types`, `Create Group Modal (legacy)`, `Dashboard Page Barrel`, `Community 58`, `Predictions Tab Component`, `Group Card (legacy)`, `Group Card Component`, `Create Group Modal`, `Group Detail Page Tests (legacy)`, `Group Detail Page (legacy)`, `Team Badge Component`, `Dashboard Page Tests (legacy)`, `Button Component`, `Google Login Button`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Bracket UI Components` to `Auth Encoding & Google OAuth`, `Predictions Tab (legacy)`, `Bracket API Router`, `App Entry & Login`, `Modal (legacy)`?**
