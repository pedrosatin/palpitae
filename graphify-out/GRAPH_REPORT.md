...
## Corpus Check
- 142 files · ~79,450 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1392 nodes · 2457 edges · 97 communities (91 shown, 6 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `653ceb8b`
...
Cohesion: 0.05
Nodes (30): ConfirmModalProps, ConfirmOptions, useConfirm(), DashboardPage(), DashboardPageProps, User, GroupDetail, GroupDetailPage() (+22 more)
...
Cohesion: 0.12
Nodes (12): Consent, getStoredConsent(), initGa(), setConsent(), Window, mockGetStoredConsent, mockSetConsent, mockTrackEvent (+4 more)
...
Cohesion: 0.15
Nodes (14): dayBounds(), exportEventsToR2(), exportRecentDays(), dd, env, events, FetchFake, { from, to, key } (+6 more)

### Community 17 - "Header Tests (legacy)"
Cohesion: 0.22
Nodes (15): base64UrlEncode(), buildAuthUrl(), exchangeCode(), generateNonce(), generatePkce(), generateState(), GoogleIdTokenClaims, GoogleUserInfo (+7 more)

### Community 18 - "Group Detail Page Tests (legacy)"
Cohesion: 0.15
Nodes (13): router, ActiveRound, pollActiveMatches(), unsubscribeByToken(), EventDims, EventType, hashUserId(), logEvent() (+5 more)
...
Cohesion: 0.16
Nodes (11): HMAC_SHA256, importHmacKey(), base64UrlDecode(), authHeaders(), Captured, ALGORITHM, importKey(), signUnsubToken() (+3 more)
...
Cohesion: 0.26
Nodes (11): ALGORITHM, importKey(), JwtPayload, signJwt(), [, body], [h, , s], payload, tamperedBody (+3 more)
...
Cohesion: 0.13
Nodes (16): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. `await hashUserId` inside `catch` aborts batch (HIGH), 1. Configurar domínio de envio no Resend, 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 2. Adicionar `RESEND_API_KEY` ao projeto, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM), 2. Query de `default_round` ignora filtros `?round=X` / `?status=X`, 3. `api/src/index.ts` — exportar handler `scheduled` (+8 more)

### Community 33 - "Web Vite Config"
Cohesion: 0.15
Nodes (16): 1. `api/migrations/0008_group_scoring_config.sql` — NOVO, Arquivos a criar, Arquivos tocados, code:sql (ALTER TABLE groups ADD COLUMN points_exact INTEGER NOT NULL ), Convenções do projeto, Decisões, Decisões tomadas, Janela de tempo (+8 more)
...
Cohesion: 0.27
Nodes (12): body, createDbMock(), createGroupsListDbMock(), createRemoveMemberDbMock(), { db }, { db, deleteRun }, { db, updateRun }, fakeEnv() (+4 more)
...
Cohesion: 0.14
Nodes (14): 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, Arquivos novos/modificados nessa branch, Arquivos relevantes, Checklist de merge, Contexto: Notificação de Nova Rodada por E-mail (+6 more)

### Community 41 - "Community 41"
Cohesion: 0.21
Nodes (14): 1. `GroupDetailPage.tsx` — adicionar a aba, 2. Segurança — XSS no template HTML (BUG), 3. `api/src/groups/router.ts`, 3. Observabilidade — nenhum evento de negócio registrado (GAP), 3. Recipient dedup uses email instead of user_id — wrong unsubscribe token (MEDIUM), 3. `StandingsTab.test.tsx` — testes, 4. Registrar cron diário no `index.ts`, 5. `makeMatch` e `mockFetch` desatualizados em `GroupPicksTab.test.tsx` (+6 more)
...
Cohesion: 0.18
Nodes (13): 2. `api/src/matches/scoring.ts`, 6. UI simplificada de palpite (quando `points_exact = 0`), 7. `.dev.vars` — não está no repositório, Arquivos a criar/modificar, code:sql (SELECT p.id, p.group_id, p.user_id,), code:ts (logEvent(c.env.AE, 'group_created', {), Como testar localmente, Decisões em aberto (+5 more)
...
Cohesion: 0.33
Nodes (6): 2. `api/src/matches/poller.ts` (implementado), 2. Novo arquivo: `api/src/matches/poller.ts`, 3. `api/src/index.ts` — handler `scheduled` (implementado), 4. `api/src/matches/router.ts` — sem mudança, 4. `api/src/matches/router.ts` — sem mudança agora, O que muda
...
- **Why does `config` connect `Brand SVG Assets` to `Bracket UI Components`, `API Routers & Auth Middleware`, `Vite Env Types`, `Architecture Decision Records`, `Login Page Barrel`, `App Entry & Login`, `Community 43`, `Fixture Sync (API Football)`, `Create Group Modal`, `Button & Header (legacy)`, `Community 49`, `Community 50`, `Community 51`, `Community 55`, `Community 54`, `Modal Component`, `Community 59`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Community 45` to `Web Test Setup`, `Dashboard Page Barrel`, `Dashboard & Group Detail Pages`, `Predictions Tab (legacy)`, `Bracket API Router`, `Group Card (legacy)`, `Community 46`, `Join Group Modal`, `Group Detail Page Tests (legacy)`, `Card (legacy)`, `Button Component`, `Google Login Button`, `Web App Entry`?**
