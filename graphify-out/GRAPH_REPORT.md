# Graph Report - palpitae  (2026-05-30)

## Corpus Check
- 104 files · ~36,552 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 714 nodes · 943 edges · 56 communities (48 shown, 8 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 8 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4489b162`
...
Cohesion: 0.05
Nodes (58): base64UrlDecode(), base64UrlEncode(), buf, decoded, encoded, original, result, buildAuthUrl() (+50 more)
...
Cohesion: 0.18
Nodes (13): buildMyPicksMap(), clearInvalidatedPicks(), getAvailableTeams(), getChildPositions(), BracketTab(), BracketTabProps, HALF_SLOTS, LEFT_BRACKET_ROUNDS (+5 more)

### Community 10 - "Predictions Tab Component"
Cohesion: 0.11
Nodes (18): byMatch, groupId, locked, match, matchId, matchIds, matchRows, membersResult (+10 more)
...
Cohesion: 0.17
Nodes (7): BracketColumnProps, ROUND_INDEX, column, { container }, slots, Pick, SlotData
...
Cohesion: 0.33
Nodes (6): FEATURE_ALLOWLISTS, FEATURE_KEYS, FeatureFlags, FeatureKey, getFeatureFlags(), hasFeatureAccess()
...
## Knowledge Gaps
- **373 isolated node(s):** `Env`, `Variables`, `app`, `ALGORITHM`, `JwtPayload` (+368 more)
...
- **Why does `config` connect `Dashboard Page Tests (legacy)` to `Architecture Decision Records`, `Create Group Modal (legacy)`, `Bracket API Router`, `App Entry & Login`, `Match Card Component`, `Community 41`, `Group Detail Page Tests (legacy)`, `Card (legacy)`, `Button Component`, `Google Login Button`, `Web App Entry`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `SlotData` connect `Group Detail Page (legacy)` to `Match Card Component`, `Group Card (legacy)`, `Community 44`, `Create Group Modal`, `Modal Component`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Modal (legacy)` to `Bracket UI Components`, `Vite Env Types`, `Predictions Tab (legacy)`, `Predictions Tab Component`, `Button & Header (legacy)`, `Icon SVG Assets`?**
