...
## Corpus Check
- 141 files · ~79,193 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1390 nodes · 2454 edges · 99 communities (93 shown, 6 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8ec99be9`
...
Cohesion: 0.14
Nodes (17): pollActiveMatches(), sendRoundReminders(), dayBounds(), exportEventsToR2(), exportRecentDays(), dd, env, events (+9 more)
...
Cohesion: 0.11
Nodes (23): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. `await hashUserId` inside `catch` aborts batch (HIGH), 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 2. `api/src/matches/poller.ts` (implementado), 2. `api/src/matches/scoring.ts`, 2. Novo arquivo: `api/src/matches/poller.ts`, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM) (+15 more)

### Community 14 - "Create Group Modal"
Cohesion: 0.13
Nodes (18): EmailError, EmailMessage, sendEmail(), body, buildEmailHtml(), buildEmailText(), crestImg(), escapeHtml() (+10 more)
...
Cohesion: 0.15
Nodes (11): GoogleLoginButton(), GoogleLogo(), mockTrackEvent, Member, MembersTab(), MembersTabProps, members, mockTrackEvent (+3 more)

### Community 18 - "Group Detail Page Tests (legacy)"
Cohesion: 0.21
Nodes (15): ActiveRound, calculatePoints(), recalculateLeaderboard(), scoreMatch(), scoreUnprocessedMatches(), batchSpy, buildFakeDb(), db (+7 more)
...
Cohesion: 0.18
Nodes (17): 1. `GroupDetailPage.tsx` — adicionar a aba, 2. Segurança — XSS no template HTML (BUG), 3. `api/src/groups/router.ts`, 3. Observabilidade — nenhum evento de negócio registrado (GAP), 3. Recipient dedup uses email instead of user_id — wrong unsubscribe token (MEDIUM), 3. `StandingsTab.test.tsx` — testes, 4. Registrar cron diário no `index.ts`, 5. `makeMatch` e `mockFetch` desatualizados em `GroupPicksTab.test.tsx` (+9 more)
...
Cohesion: 0.15
Nodes (16): API externa, Arquivos novos a criar, Checklist de merge, context.md — Tarefa atual: Result Sync & Scoring, Convenções do projeto, Cooldown / proteção de rate limit, Decisões tomadas, Fluxo acordado (+8 more)
...
Cohesion: 0.16
Nodes (6): ConfirmModalProps, ConfirmOptions, useConfirm(), Header(), HeaderProps, User
...
Cohesion: 0.15
Nodes (9): formatBRT(), body, db, imgTags, MatchRow, out, recipients, ReminderRow (+1 more)
...
Cohesion: 0.15
Nodes (13): 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, 7. `.dev.vars` — não está no repositório, Arquivos novos/modificados nessa branch, Arquivos relevantes, Contexto: Notificação de Nova Rodada por E-mail, Idempotência (sem tabela de controle) (+5 more)
...
Cohesion: 0.24
Nodes (8): unsubscribeByToken(), EventDims, EventType, hashUserId(), logEvent(), { ae, calls }, long, Point
...
Cohesion: 0.22
Nodes (9): 1. `api/migrations/0008_group_scoring_config.sql` — NOVO, Arquivos a criar, Arquivos tocados, code:sql (ALTER TABLE groups ADD COLUMN points_exact INTEGER NOT NULL ), Decisões, Janela de tempo, Próximos passos (após estabilizar o Cron), Schema relevante (+1 more)

### Community 64 - "Community 64"
Cohesion: 0.22
Nodes (9): 1. Configurar domínio de envio no Resend, 2. Adicionar `RESEND_API_KEY` ao projeto, 3. Criar módulo de notificação, Decisões em aberto, Decisões resolvidas na implementação, O que implementar, Problema, Stack do projeto (+1 more)
...
- **Why does `config` connect `Header Tests (legacy)` to `Bracket UI Components`, `Auth Encoding & Google OAuth`, `Login Page Barrel`, `Community 41`, `Community 75`, `Group Card Component`, `Community 76`, `Community 45`, `Button & Header (legacy)`, `Join Group Modal`, `Community 49`, `Group Card (legacy)`, `Community 51`, `Modal Component`, `Community 57`, `Brand SVG Assets`, `Web App Entry`, `Icon SVG Assets`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Pages Function Proxy` to `API Routers & Auth Middleware`, `Create Group Modal (legacy)`, `Dashboard & Group Detail Pages`, `Predictions Tab (legacy)`, `Bracket API Router`, `Match Card Component`, `Predictions Tab Component`, `Community 46`, `Button Component`?**
