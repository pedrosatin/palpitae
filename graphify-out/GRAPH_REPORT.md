# Graph Report - palpitae  (2026-06-23)

## Corpus Check
- 144 files · ~84,031 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1444 nodes · 2533 edges · 95 communities (89 shown, 6 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 15 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `358041da`
...
Cohesion: 0.05
Nodes (67): requireAuth, app, buildApp(), fakeEnv(), requestWithCookie(), allPicksBySlot, allPicksResult, byPos (+59 more)

### Community 2 - "Auth Encoding & Google OAuth"
Cohesion: 0.05
Nodes (55): router, ActiveRound, pollActiveMatches(), EmailError, EmailMessage, sendEmail(), body, buildEmailHtml() (+47 more)

### Community 3 - "Architecture Decision Records"
Cohesion: 0.07
Nodes (34): makeMatch(), defaultRoundIndex(), GroupMember, GroupPicksResponse, GroupPicksTab(), GroupPicksTabProps, MemberPrediction, GroupPicksResponse (+26 more)
...
Cohesion: 0.17
Nodes (18): body, BulkMatchData, BulkMockOptions, capturedSql, createBulkDbMock(), createGroupPicksDbMock(), db, fakeEnv() (+10 more)

### Community 17 - "Header Tests (legacy)"
Cohesion: 0.22
Nodes (14): HMAC_SHA256, importHmacKey(), base64UrlDecode(), base64UrlEncode(), ALGORITHM, importKey(), JwtPayload, verifyJwt() (+6 more)

### Community 18 - "Group Detail Page Tests (legacy)"
Cohesion: 0.13
Nodes (18): CacheableMatch, cacheKey, competition, competitionId, dbStartedAt, defaultRoundRow, firstOpen, MatchRow (+10 more)

### Community 19 - "Group Detail Page (legacy)"
Cohesion: 0.13
Nodes (21): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. `await hashUserId` inside `catch` aborts batch (HIGH), 1. Configurar domínio de envio no Resend, 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 2. Adicionar `RESEND_API_KEY` ao projeto, 2. `api/src/matches/scoring.ts`, 2. Novo arquivo: `api/src/matches/poller.ts`, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM) (+13 more)
...
Cohesion: 0.14
Nodes (17): API externa, Arquivos novos a criar, code:ts (function handlePreset(preset: ScoringPreset) {), context.md — Tarefa atual: Result Sync & Scoring, Convenções do projeto, Cooldown / proteção de rate limit, Decisões tomadas, Fluxo acordado (+9 more)
...
Cohesion: 0.19
Nodes (14): batchSpy, buildFakeDb(), db, FakeGroupConfig, FakeMatch, FakePrediction, groupMatch, matches (+6 more)
...
Cohesion: 0.14
Nodes (7): body, router, row, authHeaders(), Captured, token, userId
...
Cohesion: 0.21
Nodes (13): 10. O(n²) `Array.find()` scan in recipient dedup (LOW — efficiency), 3. `api/src/groups/router.ts`, 3. Observabilidade — nenhum evento de negócio registrado (GAP), 3. Recipient dedup uses email instead of user_id — wrong unsubscribe token (MEDIUM), 4. Duplicate `default_round` correlated subquery in Q1 and Q2 (MEDIUM — maintenance/correctness), 4. Registrar cron diário no `index.ts`, 5. Whitespace `RESEND_API_KEY` bypasses the guard (LOW-MEDIUM), 6. POST /unsubscribe missing RFC 8058 body validation — CSRF risk (LOW-MEDIUM) (+5 more)

### Community 38 - "Login Page Barrel"
Cohesion: 0.15
Nodes (13): 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, Arquivos novos/modificados nessa branch, Arquivos relevantes, Checklist de merge, Contexto: Notificação de Nova Rodada por E-mail (+5 more)

### Community 39 - "Bracket Tab Barrel"
Cohesion: 0.19
Nodes (13): 1. `api/migrations/0008_group_scoring_config.sql` — NOVO, 2. `StandingsTab.tsx` — componente principal, 4. `api/src/predictions/router.ts` — `GET /predictions/group`, 5. `web/src/components/CreateGroupModal/CreateGroupModal.tsx`, 6. UI simplificada de palpite (quando `points_exact = 0`), 7. Misconfig and "no rounds today" produce identical metrics (LOW), 9. `hashUserId(id)` called twice per failed send (LOW — efficiency), Arquivos a criar/modificar (+5 more)
...
Cohesion: 0.32
Nodes (9): isKnockoutPhase(), KNOCKOUT_PHASES, KNOCKOUT_PHASES_SQL, penaltyPicksActive(), calculatePoints(), penaltyBonus(), recalculateLeaderboard(), scoreMatch() (+1 more)
...
Cohesion: 0.18
Nodes (11): 2. `api/src/matches/poller.ts` (implementado), 3. `api/src/index.ts` — exportar handler `scheduled`, 3. `api/src/index.ts` — handler `scheduled` (implementado), 4. `api/src/matches/router.ts` — sem mudança, 4. `api/src/matches/router.ts` — sem mudança agora, Arquivos tocados, Decisões, Janela de tempo (+3 more)
...
Cohesion: 0.4
Nodes (5): Arquivos a criar, code:sql (ALTER TABLE groups ADD COLUMN points_exact INTEGER NOT NULL ), Pendências externas (processos manuais, sem código), Schema relevante, Testar localmente após setup

### Community 76 - "Community 76"
Cohesion: 0.4
Nodes (5): 1. `GroupDetailPage.tsx` — adicionar a aba, 3. `StandingsTab.test.tsx` — testes, 4. `index.ts` — barrel export, code:ts (setScoringPreset('classic')), Instruções de implementação
...
## Knowledge Gaps
- **450 isolated node(s):** `env`, `ctx`, `app`, `KNOWN_OAUTH_ERRORS`, `{ db, updateRun }` (+445 more)
...
- **Why does `config` connect `Button & Header (legacy)` to `Bracket UI Components`, `Community 66`, `Architecture Decision Records`, `Community 40`, `Match Card Component`, `Community 41`, `Community 44`, `Fixture Sync (API Football)`, `Community 46`, `Group Card Component`, `Community 48`, `Create Group Modal`, `Card (legacy)`, `Dashboard Page Tests (legacy)`, `Button Component`, `Web App Entry`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `AppContext` connect `API Routers & Auth Middleware` to `Web Vite Config`, `Auth Encoding & Google OAuth`, `Predictions Tab (legacy)`, `App Entry & Login`, `Join Group Modal`, `Group Detail Page Tests (legacy)`, `Modal (legacy)`, `Brand SVG Assets`?**
