...
## Corpus Check
- 140 files · ~75,827 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1315 nodes · 2358 edges · 94 communities (87 shown, 7 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1a783216`
...
Cohesion: 0.05
Nodes (55): ActiveRow, ae, db, [earliestOver, stillRelevant], errors, matchdays, runs, scoreMock (+47 more)
...
Cohesion: 0.14
Nodes (29): base64UrlEncode(), buildAuthUrl(), exchangeCode(), generateNonce(), generatePkce(), generateState(), GoogleIdTokenClaims, GoogleUserInfo (+21 more)

### Community 6 - "Predictions Tab (legacy)"
Cohesion: 0.11
Nodes (31): logRequestPerf(), RequestPerfMetrics, roundMs(), byMatch, groupId, importStatements, locked, match (+23 more)
...
Cohesion: 0.13
Nodes (11): ConfirmModalProps, Competition, CreatedGroup, CreateGroupModal(), CreateGroupModalProps, Header(), HeaderProps, User (+3 more)
...
Cohesion: 0.22
Nodes (10): requireAuth, app, buildApp(), fakeEnv(), requestWithCookie(), fakeEnv(), requestWithCookie(), router (+2 more)
...
Cohesion: 0.15
Nodes (19): 1. Configurar domínio de envio no Resend, 1. Filtro inteligente — notificar só quem NÃO palpitou ainda (PRIORIDADE ALTA), 1. `GroupDetailPage.tsx` — adicionar a aba, 2. Adicionar `RESEND_API_KEY` ao projeto, 2. Novo arquivo: `api/src/matches/poller.ts`, 2. Null user returns `round_reminders: true` instead of 404 (MEDIUM), 2. Segurança — XSS no template HTML (BUG), 3. `api/src/index.ts` — exportar handler `scheduled` (+11 more)
...
Cohesion: 0.21
Nodes (12): base64UrlDecode(), buf, decoded, encoded, original, result, ALGORITHM, importKey() (+4 more)
...
Cohesion: 0.16
Nodes (9): Consent, getStoredConsent(), initGa(), setConsent(), Window, mockGetStoredConsent, mockSetConsent, mockTrackEvent (+1 more)

### Community 26 - "Button Component"
Cohesion: 0.28
Nodes (13): signJwt(), body, createDbMock(), createGroupsListDbMock(), createRemoveMemberDbMock(), { db }, { db, deleteRun }, { db, updateRun } (+5 more)

### Community 27 - "Google Login Button"
Cohesion: 0.25
Nodes (10): HMAC_SHA256, importHmacKey(), ALGORITHM, importKey(), JwtPayload, [, body], [h, , s], payload (+2 more)
...
Cohesion: 0.14
Nodes (14): 4. Testes — gaps no `roundReminder.test.ts`, 5. Qualidade do e-mail, 6. Limites do Resend free tier, 7. `.dev.vars` — não está no repositório, Arquivos novos/modificados nessa branch, Arquivos relevantes, Checklist de merge, Contexto: Notificação de Nova Rodada por E-mail (+6 more)
...
Cohesion: 0.17
Nodes (13): 1. `api/wrangler.toml` — adicionar Cron Trigger, 1. `await hashUserId` inside `catch` aborts batch (HIGH), 1. `default_round: null/undefined` → mostra rodada 1 em vez da última, 2. Query de `default_round` ignora filtros `?round=X` / `?status=X`, 3. Duas queries D1 sequenciais por cache miss — usar `db.batch()`, 5. `makeMatch` e `mockFetch` desatualizados em `GroupPicksTab.test.tsx`, Arquivos a criar, Arquivos a modificar (+5 more)
...
Cohesion: 0.22
Nodes (11): 5. Whitespace `RESEND_API_KEY` bypasses the guard (LOW-MEDIUM), Arquivos a criar/modificar, code:ts ((env.RESEND_API_KEY ?? '').trim(),), code:ts (if (!resendApiKey.trim()) { ... }), Como testar localmente, Decisões em aberto, Decisões resolvidas na implementação, Pendências externas (não-código) (+3 more)
...
- **Why does `config` connect `Fixture Sync (API Football)` to `API Vitest Config`, `Bracket UI Components`, `Architecture Decision Records`, `Community 67`, `Dashboard Page Barrel`, `Bracket Tab Barrel`, `Predictions Tab Component`, `Community 43`, `Group Card (legacy)`, `Group Card Component`, `Community 44`, `Button & Header (legacy)`, `Community 52`, `Card (legacy)`, `Card Component`, `Community 55`, `Dashboard Page Tests (legacy)`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `AppContext` connect `Create Group Modal` to `API Routers & Auth Middleware`, `Auth Encoding & Google OAuth`, `Dashboard & Group Detail Pages`, `Predictions Tab (legacy)`, `Bracket API Router`, `App Entry & Login`, `Team Badge Component`, `Button Component`, `Brand SVG Assets`, `Pages Function Proxy`?**
