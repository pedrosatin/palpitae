# Graph Report - palpitae-pr63  (2026-07-14)

## Corpus Check
- 189 files · ~172,074 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1479 nodes · 2675 edges · 140 communities (123 shown, 17 thin omitted)
- Extraction: 97% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 64 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `08f4b778`
...
1. `AppContext` - 32 edges
2. `signJwt()` - 29 edges
3. `trackEvent()` - 27 edges
4. `config` - 23 edges
5. `🧠 Palpitae — Product Vision` - 23 edges
6. `syncFixtures()` - 20 edges
7. `logEvent()` - 20 edges
8. `apiFetch()` - 20 edges
9. `sendRoundReminders()` - 19 edges
10. `🤖 AGENTS.md — Palpitae` - 19 edges
...
Cohesion: 0.06
Nodes (60): competitions, result, router, matchGoesToPenalties(), parsePenaltyPhases(), calculatePenaltyBonus(), calculatePoints(), recalculateLeaderboard() (+52 more)
...
Cohesion: 0.11
Nodes (37): buildAuthUrl(), exchangeCode(), generateNonce(), generatePkce(), generateState(), GoogleIdTokenClaims, GoogleUserInfo, JwksKey (+29 more)
...
Cohesion: 0.08
Nodes (26): ConfirmModal(), ConfirmModalProps, ConfirmOptions, useConfirm(), GroupDetail, GroupDetailPage(), GroupDetailPageProps, parseTab() (+18 more)

### Community 5 - "Groups Router"
Cohesion: 0.07
Nodes (26): InfoHint(), InfoHintProps, InfoSvg(), computeSelectedOutcome(), formatDate(), Match, MatchCard(), MatchCardProps (+18 more)
...
Cohesion: 0.14
Nodes (24): KNOCKOUT_LABELS, roundLabel(), EmailError, EmailMessage, sendEmail(), body, buildEmailHtml(), buildEmailText() (+16 more)

### Community 8 - "Metrics Charts & Formatters"
Cohesion: 0.12
Nodes (27): CacheEntry, fetchCachedJson(), invalidateApiCache(), responseCache, loader, CompetitionType, computeStandings(), emptyStanding() (+19 more)

### Community 9 - "Metrics Router"
Cohesion: 0.11
Nodes (16): Consent, getStoredConsent(), initGa(), setConsent(), setItemMock, Window, CookieConsent(), mockGetStoredConsent (+8 more)

### Community 10 - "Group Picks & Leaderboard"
Cohesion: 0.09
Nodes (16): GroupMember, GroupPicksResponse, GroupPicksTabProps, MemberPrediction, LeaderboardTabProps, Member, UserPrediction, applyDefaultRound() (+8 more)
...
Cohesion: 0.08
Nodes (26): body, candidate, competition_id, countResult, dbStartedAt, group, groupId, GroupRow (+18 more)
...
Cohesion: 0.17
Nodes (23): AdminMetricsPage(), ArchiveResponse, buildApiCallsSeries(), buildStackedSeries(), CRON_INFO, formatAgo(), formatBytes(), formatDuration() (+15 more)

### Community 15 - "API Cache Layer"
Cohesion: 0.11
Nodes (24): ApiCompetition, ApiMatch, ApiMatchesResponse, ApiTeam, buildMatchStatement(), COMP_TRANSLATIONS, mapStatus(), resolveCanonicalScore() (+16 more)
...
Cohesion: 0.15
Nodes (18): admin, days, files, metricsRouter, monthPrefixes, now, parseDays(), runAeSql() (+10 more)

### Community 18 - "Matches Router Tests"
Cohesion: 0.19
Nodes (10): useDocumentTitle(), apiFetch(), consumeSessionExpired(), notifySessionExpired(), LoginPage(), Member, MembersTabProps, SettingsPage() (+2 more)
...
Cohesion: 0.16
Nodes (19): cacheKey, competitionId, dbStartedAt, fetchSyncNeeds(), handleGetMatches(), matchesCacheControl(), matchesOut, maybeSyncResults() (+11 more)
...
Cohesion: 0.27
Nodes (10): HMAC_SHA256, importHmacKey(), base64UrlDecode(), base64UrlEncode(), JwtPayload, verifyJwt(), signUnsubToken(), forgedId (+2 more)

### Community 23 - "Cookie Consent & GA Init"
Cohesion: 0.27
Nodes (12): authRouter, ActiveCompetition, discoverFixtures(), ActiveRound, pollActiveMatches(), scoreUnprocessedMatches(), logEvent(), app (+4 more)
...
Cohesion: 0.22
Nodes (11): DashboardPage(), DashboardPageProps, dialog, fetchSpy, mockResponse(), mockTrackEvent, user, User (+3 more)
...
Cohesion: 0.25
Nodes (13): ActiveRow, ae, buildFakeAe(), buildFakeDb(), db, [earliestOver, stillRelevant], errors, matchdays (+5 more)
...
Cohesion: 0.29
Nodes (8): app, buildApp(), fakeEnv(), requestWithCookie(), fakeEnv(), requestWithCookie(), AppContext, Variables
...
Cohesion: 0.3
Nodes (10): archive, dayKey(), fetchFake, gappyArchive, mockFetch(), overview, overviewCalls, today (+2 more)
...
Cohesion: 0.31
Nodes (6): EventDims, EventType, { ae, calls }, fakeAe(), long, Point
...
Cohesion: 0.29
Nodes (5): ADR-012: Standings Tab — Data-Driven Competition-Type Gate, Architecture Decision Records, Consequences, Context, Decision
...
## Knowledge Gaps
- **393 isolated node(s):** `PackageJson`, `result`, `buf`, `encoded`, `decoded` (+388 more)
...
- **What is the exact relationship between `PenaltyBadge` and `StandingsTab`?**
  _Edge tagged AMBIGUOUS (relation: semantically_similar_to) - confidence is low._
- **Why does `AppContext` connect `Base64 Encoding` to `Notifications & Observability`, `Group Fetch Test Mocks`, `Penalties & Scoring API`, `Community 64`, `Join Group Modal`, `Settings Page`, `Create Group Modal & Presets`, `Group Detail Page & Tabs`, `Google OAuth`, `Cookie Consent & GA Init`, `Match Poller`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `signJwt()` connect `Group Fetch Test Mocks` to `Community 64`, `Penalties & Scoring API`, `Join Group Modal`, `Base64 Encoding`, `Community 81`, `Group Detail Page & Tabs`, `Auth Router & Cookies`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `trackEvent()` connect `Predictions & Standings Tabs` to `Feature Flags & Permissions`, `Groups Router`, `Metrics Charts & Formatters`, `Metrics Router`, `Group Picks & Leaderboard`, `Results API Script`, `Crypto & OAuth Helpers`, `Modal Component`, `Button Component`, `Matches Router Tests`, `HMAC Crypto & Tokens`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._